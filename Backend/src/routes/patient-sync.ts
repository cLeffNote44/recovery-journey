...
/**
 * Create or update a goal owned by the authenticated patient.
 *
 * recoverGoalId is generated by the patient's device, so it cannot be
 * trusted for ownership: the update path is scoped to (recoverGoalId,
 * patientId), and a create that collides with another patient's row is
 * rejected by the unique constraint instead of overwriting it.
 *
 * Returns false when the recoverGoalId belongs to another patient.
 */
async function upsertOwnGoal(patientId: string, g: z.infer<typeof goalSchema>): Promise<boolean> {
  const updated = await prisma.patientGoal.updateMany({
    where: { recoverGoalId: g.recoverGoalId, patientId },
    data: {
      title: g.title,
  ...
  try {
    await prisma.patientGoal.create({
      data: {
        patientId,
...
export async function patientSyncRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/goals', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    for (const g of body.goals) {
      try {
        if (await upsertOwnGoal(patient.id, g)) {
          syncedCount++
        }
      } catch {
